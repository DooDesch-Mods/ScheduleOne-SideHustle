...
namespace SideHustle.Menu
{
    ...
    internal static class JoinBrowserView
    {
        ...
        /// <summary>Rebuild the card list from the lobby results.</summary>
        internal static void Populate(Transform content, List<LobbyRow> lobbies, Action<LobbyRow> onJoin)
        {
            ...
            Clear(content);
            foreach (var l in lobbies) BuildCard(content, l, onJoin);
        }

        private static void BuildCard(Transform content, LobbyRow row, Action<LobbyRow> onJoin)
        {
            ...
            string sub = cap
                       + (string.IsNullOrEmpty(row.GamemodeName) ? "" : "   ·   " + row.GamemodeName)
                       + (row.HasPassword ? "   ·   Locked" : "");

            var name = UIFactory.Text("name", host, card.transform, Theme.Body, TextAnchor.LowerLeft, FontStyle.Bold);
            ...
            subT.color = Theme.TextMuted; subT.raycastTarget = false; subT.horizontalOverflow = HorizontalWrapMode.Overflow;
